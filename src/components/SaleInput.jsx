...
export default function SaleInput({ onDone }) {
  ...
  // ── 메인 화면 ───────────────────────────────────────────────
  return (
    <div style={{ padding: '20px', paddingBottom: (saleMode === 'normal' ? cart.length : gachaCart.length) > 0 ? '110px' : '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {saleMode === 'normal' && cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: '70px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(10px)', borderTop: '1px solid #e2e8f0', zIndex: 50 }}>
          {cartExpanded && (
      ...
      {saleMode === 'gacha' && selectedGrade && gachaCart.length > 0 && (
        <div style={{ position: 'fixed', bottom: '70px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(10px)', borderTop: '1px solid #e2e8f0', zIndex: 50, padding: '12px 16px' }}>
