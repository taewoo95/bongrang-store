...
function ProductButton({ p, isGacha, cart, gachaCart, gachaRemaining, onAdd, onAddGacha, fmt, t }) {
  ...
  const effectiveStock = p.stock - cartQty  // 카트에 담긴 수량 제외한 실효 재고
    ...
    >
      <div>
        ...
        <div style={{ fontSize: '12px', color: outOfStock ? '#ef4444' : '#94a3b8', marginTop: '2px', fontWeight: outOfStock ? '600' : '400' }}>
          {outOfStock ? (t ? t('prod_low_stock') : '재고 없음') : `${t ? t('sale_stock_label') : '재고'} ${effectiveStock}${t ? t('pcs') : '개'}`}
        </div>
      </div>
...
export default function SaleInput({ onDone }) {
  const { t, fmt } = useLang()
  const [products, setProducts] = useState(() => getProducts().sort((a, b) => a.name.localeCompare(b.name, 'ko')))
  const refreshProducts = () => setProducts(getProducts().sort((a, b) => a.name.localeCompare(b.name, 'ko')))
  ...
  const addToCart = (product) => {
    ...
    setCart(prev => {
      ...
      const currentQty = exists?.qty || 0
      if (currentQty >= product.stock) return prev  // 재고 초과 방지
  ...
  const updateQty = (productId, delta) => setCart(prev => prev.map(c => {
    ...
    if (delta > 0 && newQty > c.product.stock) return c  // 재고 초과 방지
    return { ...c, qty: newQty }
  }).filter(Boolean))
  ...
  // 전체 금액 할인 — 개별 상품 할인 적용 후 합계에 한 번 더 적용
  const finalUnitPriceOf = (c) => Math.round(unitPriceOf(c) * (1 - (Number(cartDiscountPercent) || 0) / 100))
  ...
  const subtotalAmount = cart.reduce((sum, c) => sum + unitPriceOf(c) * c.qty, 0)
  const totalAmount = cart.reduce((sum, c) => sum + finalUnitPriceOf(c) * c.qty, 0)
  ...
  const handleSell = () => {
    ...
    for (const c of cart) {
      ...
      if (finalUnitPriceOf(c) <= 0) return alert(`"${c.product.name}"의 판매가를 확인해주세요.`)
    }
    const saleTime = new Date().toISOString()
    cart.forEach(c => {
      const unitPrice = finalUnitPriceOf(c)
      addSale({ productId: c.product.id, productName: c.product.name, qty: c.qty, unitPrice, totalPrice: unitPrice * c.qty, costPrice: c.product.costPrice, time: saleTime })
      deductStock(c.product.id, c.qty)
    })
    setDoneData({ cart: cart.map(c => ({ ...c, unitPrice: finalUnitPriceOf(c) })), totalAmount })
    setDone(true)
  }
    ...
    return (
      <div style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button onClick={() => { refreshProducts(); setCart([]); setCartDiscountPercent(0); setSelectedIds(new Set()); setBulkDiscountInput(''); setDone(false); setDoneData(null) }} style={{ background: '#6366f1', color: '#fff', borderRadius: '12px', padding: '14px 24px', fontSize: '15px', fontWeight: '600' }}>추가 판매 입력</button>
  ...
  return (
    <div style={{ padding: '20px', paddingBottom: (saleMode === 'normal' ? cart.length : gachaCart.length) > 0 ? 'calc(140px + env(safe-area-inset-bottom))' : 'calc(20px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div key={c.product.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '8px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: '14px', fontWeight: '700', color: '#6366f1', whiteSpace: 'nowrap' }}>{fmt(finalUnitPriceOf(c) * c.qty)}</span>
                    </div>
      ...
      {saleMode === 'normal' && cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', background: '#fff', borderTop: '1px solid #e2e8f0', zIndex: 99 }}>
          {cartExpanded && (
              ...
              {cart.map((c, i) => (
                <div key={c.product.id} style={{ padding: '12px 20px', borderBottom: i < cart.length - 1 ? '1px solid #f1f5f9' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '13px', fontWeight: '600', color: '#6366f1', minWidth: '64px', textAlign: 'right' }}>{fmt(finalUnitPriceOf(c) * c.qty)}</span>
          ...
          <div style={{ padding: '12px 16px', paddingBottom: 'calc(12px + 56px + env(safe-area-inset-bottom))', display: 'flex', gap: '10px', alignItems: 'center' }}>
